...
class AIMNet2Calculator(Calculator):
    ...
    def apply_afir_force(self, atoms, target_atoms, target_distance, force_constant):
       """Calculates artificial forces to attract 'target_atoms'"""
       
       current_distance = np.linalg.norm(atoms.get_positions()[target_atoms[0]] - 
                                         atoms.get_positions()[target_atoms[1]])
       force_magnitude = force_constant * (current_distance - target_distance)

       # Calculate unit vector pointing along the bond direction
       direction = (atoms.get_positions()[target_atoms[1]] -
                    atoms.get_positions()[target_atoms[0]]) / current_distance

       # Apply forces with calculated magnitude and direction
       atoms.set_forces(atoms.get_forces() + direction * force_magnitude)

    def get_combined_forces(self, atoms, target_atoms, target_distance, force_constant):
        """Combines neural network (AIMNNN) forces with AFIR forces"""
        
        # Standard Neural Network Force Calculation
        _in = self._prepere_input(atoms, atoms.get_positions())
        with torch.no_grad(), torch.jit.optimized_execution(False):
            _out = self.model(_in)  
        f = - torch.autograd.grad(_out['energy'], _in['coord'])[0]  
        nn_forces = (f * (EV2AU / ANG2BOHR))[0].flatten().cpu().numpy()

        # Apply AFIR Forces
        self.apply_afir_force(atoms, target_atoms, target_distance, force_constant)

        # Combine with NN forces (ASE expects forces on *all* atoms)
        combined_forces = nn_forces 
