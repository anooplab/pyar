...
class ANI(SF):
    def __init__(self, molecule,qc_params):
        ...
        if qc_params['gamma']:
            self.asemol.calc = GamCalcAse(self.animodel.species,self.animodel,qc_params['gamma'],molecule.fragments)
        else:
    ...
    def optimize(self, calcdetails):
        ...
        ani_logger.info('  Optimization Started...')
        stat = opt.run(fmax=0.001,steps=1000)
        if stat == False:
            ani_logger.info('    Failed!')
            return False
