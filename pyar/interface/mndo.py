...
from pyar.mlatom import stopper, simulations, models
...
class mndo_methods(models.model):
    ...
    def __init__(self, method='ODM2*', read_keywords_from_file='', save_files_in_current_directory=True, **kwargs):
        ...
        mndobin_path = os.getenv('mndobin')
        if mndobin_path is None:
            os.environ['PATH'] += os.pathsep + '~/apps/mndo2020'
            os.environ['mndobin'] = '~/apps/mndo2020/mndo2020'
            mndobin_path = os.getenv('mndobin')
            print(mndobin_path)
        if not mndobin_path:
    ...
    def predict(self, molecular_database=None, molecule=None,
        ...
        try: from pyar.mlatom import constants, data, stopper
        except:
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            for mol in molDB.molecules:
                ...
                mndobin_path = "~/apps/mndo2020/mndo2020" # replace with the actual path to mndobin
                mndoargs = [mndobin_path, '<', mndoinpfilename, '&>', mndooutfilename]
                cmd = ' '.join(mndoargs)
