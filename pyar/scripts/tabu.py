#!/usr/bin/env python3
"""Importable `pyar-tabu` entrypoint."""
...
def main():
    ...
    tabu_on = args.tabu == 'y:'
    pts = pyar.tabu.generate_points(number_of_trial_orientations, tabu_on, d_threshold=1.0)
    if args.plot:
    ...
    if args.make_configurations:
        ...
        for i, vector in enumerate(pts):
            pyar.tabu.merge_two_molecules(vector, seed, monomer, site=None,
                                          distance_scaling=1.5).mol_to_xyz(f'mol_{i:03d}.xyz')

    if args.make_composite:
        for i in range(100):
            pts = pyar.tabu.generate_points(number_of_trial_orientations, tabu_on=True, d_threshold=1.0)
            result = pyar.tabu.create_composite_molecule(seed, monomer, pts, d_scale=d_scale)
