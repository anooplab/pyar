...
class ConformerWorkflowTests(unittest.TestCase):
    ...
    def test_conformer_request_normalizes_values_without_rdkit(self):
        request = conformer._build_conformer_request(
            "CCO",
            input_format="auto",
            num_conformers="5",
            top_n="2",
        ...
        self.assertEqual(request["generation_dedup_rms"], 0.5)
        self.assertEqual(request["backend_parameters"], {"software": "xtb"})

    def test_conformer_request_validation_fails_before_rdkit(self):
        invalid_values = [
        ...
        for option, value, message in invalid_values:
            with self.subTest(option=option):
                ...
                kwargs[option] = value
                with self.assertRaisesRegex(conformer.ConformerWorkflowError, message):
                    conformer._build_conformer_request("CCO", **kwargs)
